...
func _ready() -> void:
	...
	$warrior/AnimationPlayer.play("Combat Idle")
	
	var mesh_instances = $warrior/Armature/Skeleton3D.get_children() as Array[MeshInstance3D]
...
func _physics_process(delta: float) -> void:
	...
	if navigating:
		...
		if $warrior/AnimationPlayer.current_animation != "Combat Running":
			$warrior/AnimationPlayer.play("Combat Running")
		$warrior.look_at(position - direction, Vector3.UP)
		...
		if $warrior/AnimationPlayer.current_animation != "Combat Idle":
				$warrior/AnimationPlayer.play("Combat Idle")
