...
func _input(event: InputEvent) -> void:
	...
	if GameState.active_character == character and Input.is_action_just_pressed("left_mouse_button"):
		...
		if len(result):
			if is_instance_valid(nav_point):
				nav_point.queue_free()
			$NavigationAgent3D.set_target_position(result.position)
			nav_point = load("res://navigation_point.tscn").instantiate()
			get_parent().add_child(nav_point)
			nav_point.global_transform.origin = result.position
			navigating = true
